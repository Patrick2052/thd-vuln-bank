...
from routes.upload_profile_picture.routes import profile_pic_bp
from routes.password.routes import password_bp
...
app.register_blueprint(profile_pic_bp)
...
# MOVED TO routes.upload_profile_picture
...
@app.route(
    "/latest/meta-data/iam/security-credentials/vulnbank-role", methods=["GET"]
)
...
def admin_panel(current_user):
    ...
    return render_template(
        "admin.html", users=users, pending_loans=pending_loans
    )
...
def approve_loan(current_user, loan_id):
    ...
    try:
        ...
        loan = execute_query("SELECT * FROM loans WHERE id = %s", (loan_id,))[
            0
        ]
...
def create_bill_payment(current_user):
    try:
        ...
        card_id = (
            data.get("card_id") if payment_method == "virtual_card" else None
        )
