"""
Rate Limiting Module for Vulnerable Bank Application

This module provides rate limiting functionality for API endpoints,
particularly AI chat endpoints with different limits for authenticated
...
def check_rate_limit(key, limit):
    ...
    request_count = sum(
        count for timestamp, count in rate_limit_storage[key] 
...
def get_rate_limit_status():
    ...
    Returns:
        dict: Current rate limit status for the client
    """
    cleanup_rate_limit_storage()
    client_ip = get_client_ip()
    current_time = time.time()
    ...
    unauth_count = sum(
        count for timestamp, count in rate_limit_storage[unauth_key] 
        if timestamp > current_time - RATE_LIMIT_WINDOW
    )
    ...
    if auth_header and auth_header.startswith('Bearer '):
        ...
        try:
            ...
            if user_data:
                ...
                user_count = sum(
                    count for timestamp, count in rate_limit_storage[user_key] 
                    if timestamp > current_time - RATE_LIMIT_WINDOW
                )
                ip_count = sum(
                    count for timestamp, count in rate_limit_storage[ip_key] 
                    if timestamp > current_time - RATE_LIMIT_WINDOW
                )
